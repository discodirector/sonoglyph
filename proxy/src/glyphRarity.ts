 ...
 * DUPLICATE NOTICE
 * ----------------
 * This is a byte-for-byte copy of web/src/lib/glyphRarity.ts. The bridge
 * needs to classify glyphs server-side (for OG meta-tag injection and the
 * /og/:id.png renderer) without pulling the frontend bundle. We accept the
 * duplication rather than introducing a shared workspace package — when
 * supply hits 250 we'll freeze the calibration here AND in web/, then this
 * file becomes a long-lived read-only mirror.
 *
 * Why off-chain
 * -------------
